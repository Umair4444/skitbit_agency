...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (plan && ["startup", "pro", "premium"].includes(plan.toLowerCase())) {
      ...
      const packageData = {
        ...
        priceValue:
          currency === "INR"
            ? PRICE_VALUES[planKey].inr
            : PRICE_VALUES[planKey].usd,
  ...
  const getStepContent = () => {
    ...
    if (currentStep === 1 && !order.package?.includes3DModeling) {
      return {
        ...
        subtitle:
          "We need to know if you already have a 3D model or if we should create one for you.",
        options: [
          {
            id: "yes",
    ...
    if (
      ...
      return {
        ...
        options: [
          ...
          const price =
            currency === "INR" ? option.price_inr : option.price_usd;
          const priceDisplay =
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
          ...
          {stepContent.isSummary ? (
            ...
            <div className="pb-5">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-white sm:text-3xl py-2">
                  {stepContent.title}
                </h2>
                <p className="text-neutral-400 text-base leading-relaxed sm:text-lg pb-6">
                  {stepContent.subtitle}
                </p>
              </div>

              <Card className="border-neutral-500 bg-foreground liquid-glass rounded-xl sm:rounded-2xl">
                ...
                {stepContent.options?.map((option) => (
                  ...
                  >
                    <div className="flex items-center gap-4 w-full sm:gap-5">
                      <div className="text-2xl flex-shrink-0 sm:text-3xl">
                        {option.emoji}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          {"price" in option && option.price && (
                            <div className="text-[#C6FF3A] font-bold text-sm sm:text-base">
                              {option.price}
                            </div>
                          )}
                        </div>
                        {"subtitle" in option &&
                          ...
                          option.subtitle && (
                            <div className="text-neutral-400 text-sm mt-1 sm:text-base">
                              {option.subtitle}
                            </div>
                          )}
                      </div>
      ...
      {stepContent.isSummary && (
        <div className="fixed bottom-0 left-0 right-0 liquid-glass-header border-t border-neutral-800 p-6 ">
          <div className="max-w-sm mx-auto sm:max-w-md space-y-3">
            {/* Preview button for testing */}
            <Button
              onClick={previewWhatsAppMessage}
              variant="outline"
              className="w-full h-10 text-sm border-neutral-700 text-neutral-300 hover:bg-neutral-800 bg-transparent/40"
            >
              Preview Message
            </Button>
            ...
            <Button
              ...
              className="w-full h-16 md:h-14 text-base font-semibold bg-[#C6FF3A] text-black hover:bg-[#C6FF3A]/90 rounded-xl shadow-lg shadow-[#C6FF3A]/20 sm:text-xl sm:rounded-2xl"
            >
